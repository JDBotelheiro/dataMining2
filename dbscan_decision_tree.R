library(pROC)
dados <- read.table("dados_clusters_dbscan.txt")
str(dados)
...
tree.1 <- rpart(cluster ~ Age + Sex + On.thyroxine + Query.on.thyroxine + On.antithyroid.medication + Sick + Pregnant + Thyroid.surgery + I131.treatment + Query.hypothyroid + Query.Hiperthyroid + Lithium + Goitre + Tumor + Hypopituitary + Psych + TSH + T3 + TT4 + T4U + FTI, data=dados, method="class")
tree.1
...
#Confusion matrix
conf.matrix <- table(dados$cluster , predict(tree1.pruned,type="class"))
print(conf.matrix)

pred.my <- as.numeric(predict(tree.1, dados, type = "class"))
table(pred.my, dados$cluster)
