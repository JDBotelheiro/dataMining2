...
library(dbscan)
library(factoextra)
dados <- read.table("sickclasses.txt")
View(dados)
dados2 <- dados[,1:21]
View(dados2)
...
abline(h = 32 , lty = 2) #eps=32 eh o valor otimal quando minpoints=3

set.seed(123)
# fpc package
res.fpc <- fpc::dbscan(dados2, eps = 32, MinPts = 3)
res.fpc
# dbscan package
res.db <- dbscan::dbscan(dados2, 32, 3)
res.db
...
str(dados)
write.table(dados, file = "sick_clusters_dbscan.txt")
