...
tabela <- wardclustersick
str(tabela)
tabela$novodiag <- as.factor(tabela$novodiag)
str(tabela)

tree.1 <- rpart(novodiag ~ Age + Sex + On.thyroxine + Query.on.thyroxine + On.antithyroid.medication + Sick + Pregnant + Thyroid.surgery + I131.treatment + Query.hypothyroid + Query.Hiperthyroid + Lithium + Goitre + Tumor + Hypopituitary + Psych + TSH + T3 + TT4 + T4U + FTI, data=tabela, method="class")
tree.1
...
#Confusion matrix
conf.matrix <- table(tabela$novodiag, predict(tree1.pruned,type="class"))
print(conf.matrix)
